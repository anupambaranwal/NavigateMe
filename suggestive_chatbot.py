...
def directions_suggestion(origin_text, destination_text):
    """Get Directions."""
    try:
        bot_response = MapApiModule.direction(origin_text, destination_text)
...
def geocoding_suggestion(location_text):
    """Find a place."""
    try:
        bot_response = MapApiModule.geocoding(location_text)
    except Exception as e:
        logging.debug(e)
        return None
    logging.debug(bot_response)
...
def timezone_suggestion(location_text, tz_option="time"):
    ...
    try:
        timezone_name, time_in_tz = MapApiModule.timezone(location_text)
    except Exception as e:
        logging.debug(e)
        return None
...
def elevation_suggestion(location_text, location_comp_text=None):
    ...
    try:
        location_elev = MapApiModule.elevation(location_text)
    except Exception as e:
        logging.debug(e)
        return None
    if location_comp_text:
        try:
            location_comp_elev = MapApiModule.elevation(location_comp_text)
        except Exception as e:
            logging.debug(e)
            return None
...
def mapsstatic_suggestion(location_text, **kwargs):
    ...
    try:
        bot_response = MapApiModule.mapsstatic(location_text, **kwargs)
    except Exception as e:
        logging.debug(e)
        return None
    logging.debug(bot_response)
